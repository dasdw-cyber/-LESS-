...
# ======================= 配置区域 =======================
# 输入数据路径 (Step 3 生成的文件)
DATA_PATH = r"E:\叶绿素反演-李文娟老师论文\适配LESS的prosail数据\6波段\PROSAIL_LUT_50k_18Params.csv"

# 模型保存目录
MODEL_SAVE_DIR = r"E:\叶绿素反演-李文娟老师论文\适配LESS的model\6波段"
...
def train_inversion_models():
    ...
    # 2. 提取特征 (X)
    # 筛选列名包含 '_k' 的列 (应该是 18 个核系数)
    feature_cols = [c for c in df.columns if '_k' in c]

    if len(feature_cols) != 18:
        print(f"⚠️ 警告: 识别到的特征列数量为 {len(feature_cols)}，预期为 18。")
    ...
    for target in TARGETS:
        ...
        for i in range(N_ENSEMBLE):
            ...
            model = MLPRegressor(hidden_layer_sizes=(5,),
                                 activation='tanh',
                                 solver='lbfgs',  # 🌟 修改 1: 换用 lbfgs，极大地增强小网络的收敛能力
                                 max_iter=3000,  # 🌟 修改 2: 增加最大迭代次数，给 lbfgs 足够的收敛空间
                                 early_stopping=False,  # 🌟 修改 3: 强制关闭早停，逼迫网络去寻找特征规律
                                 random_state=i)
...
def plot_validation(y_true, y_pred, target_name, r2, rmse):
    ...
    if len(y_true) > 5000:
        # 优化 1: 更换为高对比度的科学色带 'viridis'。备选方案: 'turbo', 'jet', 'plasma'
    ...
    plt.title(f'{target_name.upper()} Ensemble Validation\n$R^2$={r2:.3f}, RMSE={rmse:.3f}')
